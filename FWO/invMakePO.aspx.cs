...
namespace FRDP
{
    public partial class invMakePO : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string MakeRFQ(string h, string i, string con, string thisid, string reqID)
        {
            string ret = "";
            ...
            if (thisid == "0")
            {
                ...
                string tblInvReqNo = "";
                //string vanders = head["Head"]["vnderslist"];
                using (DBDataContext db = new DBDataContext())
                {
                    db.sp_tblInvReqMainPO_Insert(Convert.ToInt32(Convert.ToString(Convert.ToString(((HttpCookie)HttpContext.Current.Request.Cookies["Emp_Id"]).Value))), Convert.ToInt32(con), Convert.ToDecimal(head["Head"]["StoreID"]), Convert.ToString(head["Head"]["StoreName"]), Convert.ToString(head["Head"]["Subject"]), Convert.ToString(head["Head"]["Description"]), ref tblInvReqMainID, ref tblInvReqNo, Convert.ToDecimal(reqID));
                    List<tblInvReqSub> l = new List<tblInvReqSub>();
                    //List<tblInvReqMainVender> lv = new List<tblInvReqMainVender>();
                    ...
                    //foreach (var item in vanders.Split('½'))
                    //{
                    //    tblInvReqMainVender tv = new tblInvReqMainVender();
                    //    tv.tblInvReqMainID = tblInvReqMainID;
                    //    tv.tblVenderID = Convert.ToInt32(item);
                    //    lv.Add(tv);
                    //}
                    //db.tblInvReqMainVenders.InsertAllOnSubmit(lv);
