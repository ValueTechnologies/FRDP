<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.Master" AutoEventWireup="true" CodeBehind="accOAV.aspx.cs" Inherits="FRDP.accOAV" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="row-fluid">
        <div class="span12">
            ...
            <div class="box gradient">
                ...
                <div class="title">
                    ...
                    <h4>
                        ...
                        <span>Opening & Adjustment Vouchers</span>
                    </h4>
            ...
            <td>&nbsp;</td>
            <td>
                <input name="typ" id="rdPay" type="radio" checked="checked" /><label for="rdPay">Opening Balances Voucher</label></td>
            <td>
                <input name="typ" id="rdRec" type="radio" /><label for="rdRec">Adjustment Vouchers</label></td>
        </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td colspan="2">
                                    <table style="width: 100%;">
                                        <tr>
                                            <td>
                                                <%--<div id="JVNO" title="OBV/<%:Fn.GetRecords("SELECT COUNT(isnull(VoucherID,0))+1 AS VNo FROM TblVoucherHDR WHERE (CompanyID = "+Request.Cookies["br"].Value+") AND ({ fn MONTHNAME(EntryDate) } + ' ' + CONVERT(varchar, YEAR(EntryDate)) = '"+Convert.ToDateTime(Session["ActMth"]).ToString("MMMM yyyy")+"') AND (VoucherNo LIKE N'OBV%')")[0] %>" style="float: left; font-weight: bold; margin-left: 5px;">Voucher No. OBV/<%:Fn.GetRecords("SELECT COUNT(isnull(VoucherID,0))+1 AS VNo FROM TblVoucherHDR WHERE (CompanyID = "+Request.Cookies["br"].Value+") AND ({ fn MONTHNAME(EntryDate) } + ' ' + CONVERT(varchar, YEAR(EntryDate)) = '"+Convert.ToDateTime(Session["ActMth"]).ToString("MMMM yyyy")+"') AND (VoucherNo LIKE N'OBV%')")[0] %></div>--%>
                                        ...
                                        <tr>
                                            <td>
                                                <span id="psource">Reason :</span>
                                                <input id="txPayee" type="text" style="width: 300px" />
                                            </td>
                                            <td style="width:320px;">&nbsp</td>
                                            <td>
                                             <strong>Ref. # :</strong><asp:TextBox ID="txCheqNo" runat="server" ClientIDMode="Static"></asp:TextBox>
        ...
        $(document).ready(function () {
            ...
            $('#rdPay').change(function () {
                if ($('#rdPay').attr("checked") == "checked") {
                    $.ajax({
                        url: "accgetVNo.ashx/?ID=" + $('#ddCampus').val() + "|" + $('#AcYearID').attr("tag") + "|OBV",
                        ...
                        success: function (responseText) {
                            ...
                            $('#vTitleDisplay').text('Opening Balances Voucher');
                        }
                    });
                }
            });
            $('#rdRec').change(function () {
                if ($('#rdRec').attr("checked") == "checked") {
                    $.ajax({
                        url: "accgetVNo.ashx/?ID=" + $('#ddCampus').val() + "|" + $('#AcYearID').attr("tag") + "|ADV",
                        context: document.body,
                        success: function (responseText) {
                            $('#JVNO')[0].innerHTML = "Voucher No. " + responseText;
                            $('#JVNO')[0].title = responseText;
                            ...
                            $('#vTitleDisplay').text('Adjustment Vouchers');
                        }
                    });
                }
            });
            ...
            $('#btnprtP').click(function () {
                ...
                var chno = "";
                chno = '<span>'  + '  Ref. # : ' + $('#txCheqNo').val() + '</span>';
            ...
            $('#btnSaveV').live('click', function () { //get data from table
                if (vdcheck()) {
                    ...
                    $.ajax({
                        url: "accpostJVBV.ashx/?ID=" + encodeURIComponent(dataIN + "¦" + $('#txDate').val() + "¦" + $('#JVNO').attr("title") + "¦" + $.cookie('UserID') + "¦" + $('#ddCampus').val() + "¦" + $('#txJMemo').val() + "¦" + $('#ddAYear').val() + "¦NULL¦" + "¦"  + $('#txCheqNo').val() + "¦" + $('#txPayee').val()),
                        context: document.body,
                        success: function (responseText) {
                            if (responseText != "[]") {
