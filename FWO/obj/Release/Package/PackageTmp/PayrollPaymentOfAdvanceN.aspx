<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.Master" AutoEventWireup="true" CodeBehind="PayrollPaymentOfAdvanceN.aspx.cs" Inherits="FRDP.PayrollPaymentOfAdvanceN" %>
            ...
            <div class="box gradient">
                ...
                <div class="content noPad" style="padding: 1px;">
                    ...
                    <form class="form-horizontal" action="#">
                        ...
                        <div class="form-row row-fluid clsShow" style="display:none;">
                            <div class="span6">
                                <label class="form-label span4" for="normal">Payment Date</label>
                                <input id="txtPaymentDate" type="text" class="txtcs frmCtrl span6" />
                            </div>
                        </div>
                        ...
                        <div class="form-row row-fluid clsShow" style="display:none;">
                            <div class="span6">
                                <label class="form-label span4" for="normal">Payment Amount</label>
...
///////////////////////////////////////////////SEARCH START
            ...
            $('.btnEdit').live('click', function (e) {
                ...
                $('#btnSave').attr('tag', tg);
                $('#ddlTitleOfAdvance').attr('tag', tg);
                EmpDisplayFunc(tg);
                LoadEmployeePreviousAdvances();
            });
            ////////////////////SEARCH START
            ...
            $('#txtAmount').numeric();
            $('#ddlTitleOfAdvance').bind('change', function () {
                $('#btnSave').attr('tag', 0);
                LoadEmployeePreviousAdvances();
            });
            LoadAdvanceTitle();
            ...
            $('.PaymentEmployee').live('click', function (e) {
                e.preventDefault();
                $('#btnSave').attr('tag', $(this).attr('tag'));
                $(this).parent().prev().prev().prev().css('background-color', 'rgba(106, 125, 104, 0.32)');
                $(this).parent().prev().prev().css('background-color', 'rgba(106, 125, 104, 0.32)');
                $(this).parent().prev().css('background-color', 'rgba(106, 125, 104, 0.32)');
                $(this).parent().css('background-color', 'rgba(106, 125, 104, 0.32)');

                $('.clsShow').show('slow');
            });
            function LoadEmployeePreviousAdvances() {
                $('#tbPreviousAdvances tbody').html('');
                $.ajax({
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    url: "PayrollPaymentOfAdvance.aspx/SearchPrevousData",
                    data: "{ 'EmpID' : '" + $('#ddlTitleOfAdvance').attr('tag') + "', 'AdvanceId' : '" + $('#ddlTitleOfAdvance').val() + "'}",
                    async: false,
                    success: function (responseText) {
                        var jData = $.parseJSON(responseText.d), out = "";
                        $.each(jData, function (i, item) {
                            out = out + '<tr><td style="text-align : left;">' + item.Srno + '</td><td style="text-align : left;">' + item.Amount + '</td><td style="text-align : left;">' + item.Balance + '</td><td> <button type="button" class="btn btn-link PaymentEmployee" tag="' + item.AdvanceDetailID + '">Payment </button></td> </tr>';
                        });
            ...
            $('#btnSave').live('click', function () {
                if ($('#txtPaymentDate').val().trim() == "") {
                    alertR('Please enter Payment Date...');
                    return;
                }
                ...
                if ($('#btnSave').attr('tag') == undefined) {
                    alertR('Please select which advance payment you are paying....');
                    return;
                }
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    url: "PayrollPaymentOfAdvance.aspx/SavePayment",
                    data: "{ 'AdvanceID' : '" + $('#ddlTitleOfAdvance').val() + "', 'EmpID' : '" + $('#ddlTitleOfAdvance').attr('tag') + "', 'AdvanceDetailid' : '" + $('#btnSave').attr('tag') + "' , 'Date' : '" + $('#txtPaymentDate').val().trim() + "', 'Amount' : '" + $('#txtAmount').val() + "' }",
                    success: function (response) {
                        LoadEmployeePreviousAdvances();
                        $('#txtAmount').val('');
                        $('#txtPaymentDate').val('');
