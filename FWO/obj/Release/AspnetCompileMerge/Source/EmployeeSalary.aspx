<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.Master" AutoEventWireup="true" CodeBehind="EmployeeSalary.aspx.cs" Inherits="FRDP.EmployeeSalary" %>
...
<td>' + itm.BPay + '</td>\
            ...
            $('.clsddlShfit').live('change', function () {
                var $dl = $(this);
                $.ajax({
                    type: "POST",
                    url: "EmpReg.ashx/?ID=30ǁ" + $dl.attr('eid') + "ǁ" + $dl.val(),
                    success: function (response) {
                        if (response == "1") {
            ...
            $('.clsSalary').live('focusout', function () {
                var $dl = $(this);
                //alertG($dl.attr('t') + '= ' + $dl.val());
                if ($dl.attr('t') != $dl.val()) {

                    // alert("called");
                    $.ajax({
                        type: "POST",
                        url: "EmpReg.ashx/?ID=31ǁ" + $dl.attr('eid') + "ǁ" + $dl.val(),
                        success: function (response) {
                            if (response == "1") {
                                alertG('Record Updated..!');
                                $dl.attr('t', $dl.val());
