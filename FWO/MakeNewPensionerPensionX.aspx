<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.Master" AutoEventWireup="true" CodeBehind="MakeNewPensionerPensionX.aspx.cs" Inherits="FRDP.MakeNewPensionerPensionX" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#btnSearch').bind('click', function () {
                ...
                $.ajax({
                    ...
                    data: "{ 'FileNO' : '" + $('#txtFileNo').val() + "', 'name' : '" + $('#txtName').val() + "', 'DepartmentID' : '" + $('#ddlDept').val() + "' , 'DesignationID' : '" + $('#ddlDesignation').val() + "'}",
                    success: function (response) {
                        var jData = $.parseJSON(response.d), out = "";

                        $.each(jData, function (i, item) {
                            out = out + '<tr><td> ' + item.sno + '</td> <td  style="text-align : left;"> ' + item.FileNo + '</td>  <td style="text-align : left;"> ' + item.Full_Name + '</td>  <td  style="text-align : left;"> ' + item.FatherName + '</td> <td  style="text-align : left;"> ' + item.CNIC + '</td> <td  style="text-align : left;"> ' + item.DepartmentName + '</td> <td  style="text-align : left;"> ' + item.Designation + '</td> <td> <button type="button" class="btn btn-link SearchedEmployee" tag="' + item.User_ID + '">Detail </button></td> </tr>';
                        });
            ...
            $('#txtTimeToTimeGrossPension').live('keyup', CalculateValues);

            $('#txtBasicPay').live('keyup', CalculateValues);
            ...
            function CalculateValues() {
                ...
                var TotalGrossPension = 0;
                if ($('#txtTimeToTimeGrossPension').val().trim() == "") {
                    TotalGrossPension = GrossPension;
                }
                else {
                    TotalGrossPension = parseInt(GrossPension, 10) + parseInt($('#txtTimeToTimeGrossPension').val(), 10);
                }

                $('#txtTotalGrossPension').val(TotalGrossPension);
                $('#txtMonthlyPension').val(Math.round(TotalGrossPension * 0.65).toFixed(2));

                var Commutation = (TotalGrossPension * 0.35);
            ...
            $('#txtCashablePercentage').live('keyup', function () {
                ...
                if ($('#txtMedicalAllowance').val().trim() != "") {
                    MedicalAllowance = $('#txtMedicalAllowance').val();
                }
                $('#txtCashableAmount').val(parseInt(Math.round((monthlyPension * Percentage) / 100).toFixed(2), 10) + parseInt(MedicalAllowance, 10));
            });
        });
        ...
        $('body').on({
            click: function () {
                ...
                $('#lblFileNo').html($(this).parent().prev().prev().prev().prev().prev().prev().html());
                ...
                <div class="content" id="dvSearchEmployees">
                    ...
                    <form class="form-horizontal" action="#">
                        <div class="form-row row-fluid">
                            <div class="span6">
                                <label class="form-label span4" for="normal">File No</label>
                                <input id="txtFileNo" type="text" class="txtcs  span7" />
                            </div>
                            ...
                            <div class="span12">
                                <div style="border-color: rgb(221, 221, 221); border-width: 1px; border-style: solid;">
                                    <table style="width: 100%; background-color: whitesmoke;">
                                        <tr>
                                            <td style="font-weight: bold;">File No : 
                                            </td>
                                            <td>
                                                <label id="lblFileNo" for="normal"></label>
                                            </td>
                                            <td style="width: 5%;"></td>
