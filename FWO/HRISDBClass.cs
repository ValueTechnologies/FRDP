...
namespace FRDP
{
    ...
    public class HRISDBClass
    {

        public SqlConnection CN = new SqlConnection();
        public string SQL_Str = System.Configuration.ConfigurationManager.ConnectionStrings["VD_DB_ConnectionString"].ToString();
        // public string SQL_Str =  "Data Source=.\\;Initial Catalog=RDP_MIS;User ID=sa;Password=mzislam";
        //public string SQL_Str = "Data Source=localhost;Initial Catalog=MIS_RDP;User ID=arrows;Password=java123";//"Data Source=localhost;Initial Catalog=TaskMgr_DB;User ID=arrows;Password=java123";//
        ...
        public void SendEmail(string txtTo, string txtSubject, string txtBody, string txtFrom, DataTable cc)
        {
            try
            {
                ...
                if (cc !=null && cc.Rows.Count > 0)
                {
                    for (int i = 0; i < cc.Rows.Count; i++)
                    {
                        mailMsg.CC.Add(Convert.ToString(cc.Rows[i]["Email"]));
                    }
                }
        ...
        public string GetRecordCandidate(int CandidateID) 
        {
            string outputdv = string.Empty;
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("usp_JobOfferLetter", CN);
                cmd.CommandType = CommandType.StoredProcedure;
                SqlParameter parm = new SqlParameter("@outputDiv", SqlDbType.VarChar, 100000);
                ...
                outputdv = Convert.ToString(cmd.Parameters["@outputDiv"].Value);
                cmd.Dispose();
            }
            catch (Exception ex)
            {
