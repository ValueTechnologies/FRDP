<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.Master" AutoEventWireup="true" CodeBehind="accbV.aspx.cs" Inherits="FRDP.accbV" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="row-fluid">
        <div class="span12">
            ...
            <td>&nbsp;</td>
            <td>
                <input name="typ" id="rdPay" type="radio" checked="checked" /><label for="rdPay">Bank Payment Voucher</label></td>
            <td>
                <input name="typ" id="rdRec" type="radio" /><label for="rdRec">Bank Receipt Voucher</label></td>
        </tr>
                            <tr>
                            ...
                            <tr>
                                <td></td>
                                <td id="vTitleDisplay" colspan="2" style="text-align: center; font-variant: small-caps; font-size: x-large; font-weight: bold;">
                            ...
                            <tr>
                                ...
                                <td colspan="2">
                                    <table style="width: 100%;">
                                        <tr>
                                            <td>
                                                <%--<div id="JVNO" title="BPV/<%:Fn.GetRecords("SELECT COUNT(isnull(VoucherID,0))+1 AS VNo FROM TblVoucherHDR WHERE (CompanyID = "+Request.Cookies["br"].Value+") AND ({ fn MONTHNAME(EntryDate) } + ' ' + CONVERT(varchar, YEAR(EntryDate)) = '"+Convert.ToDateTime(Session["ActMth"]).ToString("MMMM yyyy")+"') AND (VoucherNo LIKE N'BPV%')")[0] %>" style="float: left; font-weight: bold; margin-left: 5px;">Voucher No. BPV/<%:Fn.GetRecords("SELECT COUNT(isnull(VoucherID,0))+1 AS VNo FROM TblVoucherHDR WHERE (CompanyID = "+Request.Cookies["br"].Value+") AND ({ fn MONTHNAME(EntryDate) } + ' ' + CONVERT(varchar, YEAR(EntryDate)) = '"+Convert.ToDateTime(Session["ActMth"]).ToString("MMMM yyyy")+"') AND (VoucherNo LIKE N'BPV%')")[0] %></div>--%>
                                                <div id="JVNO"  style="float: left; font-weight: bold; margin-left: 5px;"></div>
                                            </td>
                                            <td style="width:100px;">&nbsp</td>
                                            <td>
                                                <strong style="margin-left: 10px;">Date</strong> : &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<asp:TextBox ID="txDate" runat="server" Font-Size="11px" BorderStyle="None" ClientIDMode="Static" ReadOnly="True"></asp:TextBox>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <input id="txPayee" type="text" style="width: 300px" />
                                            </td>
                                            <td style="width:320px;">&nbsp</td>
                                            <td>
                                                <select id="ddlChequeType" style="border:none; width:60px;margin-bottom: 5px;">
                                                    <option>CH</option>
                                                     <option>DD</option>
                                                     <option>PO</option>
                                                </select> <strong>Ref. # :</strong><asp:TextBox ID="txCheqNo" runat="server" ClientIDMode="Static"></asp:TextBox>
                                                ...
                                                <table id="test-table" style="border-collapse: collapse;" class="responsive table">
                                                    <tr>
                                                        <td style="width: 579px">
                                                            <input class="AcName" placeholder="Account Head" type="text" style="border-width: thin; width: 100%;  font-weight: bold;" /><input class="Dsc" placeholder="description" type="text" value="" style="border-width: thin; width: 100%; font-size: smaller; " /></td>
                                        ...
                                        <tr>
                                            <td style="border-top-style: dotted; border-top-width: thin;" colspan="3">
                                        ...
                                        <tr>
                                            <td colspan="3">Voucher Description
                                <br />
        ...
        $(document).ready(function () {
            ...
            $('#rdPay').change(function () {
                if ($('#rdPay').attr("checked") == "checked") {
                    $.ajax({
                        url: "accgetVNo.ashx/?ID=" + $('#ddCampus').val() + "|" + $('#AcYearID').attr("tag") + "|BPV",
                        ...
                        success: function (responseText) {
                            ...
                            $('#vTitleDisplay').text('Bank Payment Voucher');
                        }
                    });
                }
            });
            $('#rdRec').change(function () {
                if ($('#rdRec').attr("checked") == "checked") {
                    $.ajax({
                        url: "accgetVNo.ashx/?ID=" + $('#ddCampus').val() + "|" + $('#AcYearID').attr("tag") + "|BRV",
                        context: document.body,
                        success: function (responseText) {
                            $('#JVNO')[0].innerHTML = "Voucher No. " + responseText;
                            $('#JVNO')[0].title = responseText;
                            ...
                            $('#vTitleDisplay').text('Bank Receipt Voucher');
                        }
                    });
                }
            });
            ...
            $('.AcName').live('focusout', function (e) {
                var obj = $(this);
                $(this).attr("value", $(this).val());
                var v = xList1[jQuery.inArray($(this).val(), xList2)];
            ...
            $('#btnprtP').click(function () {
                ...
                var payee = "";
                if ($('#rdPay').attr("checked") == "checked") {
                    payee = 'Payee : ' + $('#txPayee').val();
                }
                else {
                    payee = 'Source:' + $('#txPayee').val();
                }
                $('#txPayee').parent().html('<span>'+payee+'</span>');
                var chno = "";
                chno = '<span>' + $('#ddlChequeType').val() + '  Ref. # : ' + $('#txCheqNo').val() + '</span>';
                $('#txCheqNo').parent().html(chno);
                var dte = '<span> Date : ' + $('#txDate').val() + '</span>';
                $('#txDate').parent().html(dte);
            ...
            $('#btnSaveV').live('click', function () { //get data from table
                if (vdcheck()) {
                    ...
                    $.ajax({
                        url: "accpostJVBV.ashx/?ID=" + encodeURIComponent(dataIN + "¦" + $('#txDate').val() + "¦" + $('#JVNO').attr("title") + "¦" + $.cookie('UserID') + "¦" + $('#ddCampus').val() + "¦" + $('#txJMemo').val() + "¦" + $('#ddAYear').val() + "¦NULL¦" + "¦" + $('#ddlChequeType').val() + " │ " + $('#txCheqNo').val() + "¦" + $('#txPayee').val()),
                        context: document.body,
                        success: function (responseText) {
                            if (responseText != "[]") {
