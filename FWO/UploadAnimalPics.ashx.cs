...
{
    /// <summary>
    /// Summary description for UploadAnimalPics
    /// </summary>
    public class UploadAnimalPics : IHttpHandler, IRequiresSessionState
    {
            ...
            try
            {
                ...
                if (context.Request.Files.Count > 0 && benID > 0)
                {
                    ...
                    for (int i = 0; i < SelectedFiles.Count; i++)
                    {
                        ...
                        string FileName = context.Server.MapPath("~/Uploads/AnimalPhotos/" + PostedFile.FileName);
                        string Path = context.Server.MapPath("~/Uploads/AnimalPhotos/");
                        FileInfo fi = new FileInfo(FileName);
                        decimal fileID =Convert.ToDecimal(d[0]);

                        using (DBDataContext db = new DBDataContext())
                        {
                            var o = db.tblAnimals.Where(v => v.ID == fileID).FirstOrDefault();
                            if (o != null)
                            {
                                if (d[1]=="Front")
                                {
                        ...
                        PostedFile.SaveAs(Path + d[1]+ Convert.ToString(fileID) + fi.Extension);
                        ...
                        {
                            Bitmap Thumbnail = CreateThumbnail(Path +d[1]+ Convert.ToString(fileID) + fi.Extension, 32, 32);
                            string SaveAsThumbnail = System.IO.Path.Combine(context.Server.MapPath("~") + "/Uploads/AnimalPhotos/", d[1]+"A" +Convert.ToString(fileID) +  fi.Extension);
                            Thumbnail.Save(SaveAsThumbnail);
                        }

                        if (fi.Extension.ToUpper() == ".JPEG" || fi.Extension.ToUpper() == ".JPG" || fi.Extension.ToUpper() == ".BMP" || fi.Extension.ToUpper() == ".PNG" || fi.Extension.ToUpper() == ".GIF")
                        {
                            Bitmap Thumbnail = CreateThumbnail(Path + d[1]+Convert.ToString(fileID) + fi.Extension, 75, 75);
                            string SaveAsThumbnail = System.IO.Path.Combine(context.Server.MapPath("~") + "/Uploads/AnimalPhotos/", d[1]+"B" +Convert.ToString(fileID) +  fi.Extension);
                            Thumbnail.Save(SaveAsThumbnail);
                        }
